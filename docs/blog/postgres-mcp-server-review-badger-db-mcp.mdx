...
Use any local or hosted Postgres (Docker, cloud, or tunneled) and pass a `postgres://` DSN or configure `dbhub.toml`.
...
- **Connection and query timeouts**: Prevent runaway queries and connection hangs with `connection_timeout` and `query_timeout` settings. If `query_timeout` is omitted, a **5-minute** client-side default applies for supported engines (not SQLite).
...
**Electric Elephant** is a zero-dependency, token efficient MCP server for PostgreSQL with guardrails.
...
- **Multi-instance Postgres with consistent guardrails**: Connect several PostgreSQL sources via TOML with per-tool settings. Guardrails include read-only mode, row limits, connection/query timeouts, and (Electric Elephant) fail-closed PII/clinical heuristics on `execute_sql` unless `allow_access_to_pii_data` is explicitly enabled.
...
quadrantChart
    ...
    y-axis Postgres-only --> Many database engines
    Supabase MCP Server: [0.1, 0.15]
    MCP Toolbox: [0.3, 0.92]
    Electric Elephant: [0.85, 0.12]
```

**If you're an AI-assisted developer using PostgreSQL**, Electric Elephant is built for you. It's optimized for the local development workflow—minimal token overhead and no bundled SaaS authentication.
...
3. **Electric Elephant** (this article) - Zero-dependency, token efficient MCP server for PostgreSQL
