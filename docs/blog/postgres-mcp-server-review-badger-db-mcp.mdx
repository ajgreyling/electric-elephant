---
title: "Postgres MCP Server Review - Electric Elephant Design Explained"
description: "A maintainer's deep-dive into Electric Elephant—a zero-dependency, token efficient MCP server for PostgreSQL, MySQL, SQL Server, MariaDB, and SQLite with consistent guardrails."
---

_Last updated: Dec 28, 2025_

This is the third in a series reviewing Postgres MCP servers. Here we—the Electric Elephant maintainers—explain the design behind [Electric Elephant](https://github.com/ajgreyling/electric-elephant), a zero-dependency, token efficient MCP server that connects AI assistants to PostgreSQL, MySQL, MariaDB, SQL Server, and SQLite. We'll cover our design decisions, trade-offs, and where Electric Elephant falls short compared to alternatives.

- **GitHub Stars**

    [![Star History Chart](https://api.star-history.com/svg?repos=ajgreyling/electric-elephant&type=date&legend=top-left)](https://www.star-history.com/#ajgreyling/electric-elephant&type=date&legend=top-left)
...
```bash
npx electric-elephant --dsn "postgres://user:password@localhost:5432/dbname"
```

One command, no configuration file required. Unlike MCP Toolbox which requires a YAML config file upfront, Electric Elephant works directly with a DSN. 
Configuration files are optional—use them when you need advanced settings.

For those who want to try Electric Elephant without setting up a database first:

```bash
npx electric-elephant --demo
```

This starts Electric Elephant with a bundled SQLite employee database—useful for exploring the MCP integration before connecting to your own databases.

## Tools

Electric Elephant provides two built-in tools:
...
Token efficiency is Electric Elephant's key design objective. This matters for two reasons:
...
Electric Elephant loads only 2 built-in tools with 1.4k tokens by default:
...
> /context
  ⎿
     ...
     MCP tools · /mcp
     └ mcp__electric-elephant__execute_sql: 607 tokens
     └ mcp__electric-elephant__search_objects: 817 tokens
```
...
|------------|----------------|----------------|
| Electric Elephant | **1.4k** (2 tools) | **607** (1 tool) |
...
| Scenario | Traditional Approach | Electric Elephant Approach |
|----------|---------------------|------------------------|
...
Electric Elephant supports advanced settings via TOML configuration:

- **Read-only mode**: Keyword filtering to restrict SQL execution. This is a limitation—Supabase MCP guarantees read-only via a dedicated `supabase_read_only_user`, while Electric Elephant requires user discipline to supply a read-only database user for true guarantee.

- **Row limiting**: Prevent accidental large data retrieval with `max_rows`. If your query already has a `LIMIT` clause, Electric Elephant uses the smaller value.
...
**What's missing**: Authentication. Unlike MCP Toolbox which provides Google Auth, or Supabase MCP which integrates OAuth with its hosted service, Electric Elephant doesn't provide any auth yet (we're planning to support vendor-neutral authentication such as Keycloak).

## Workbench

Electric Elephant includes a [built-in web interface](/workbench/overview) with two main features:
...
1. **Tool Execution**: Run database tools directly from your browser—useful for testing and debugging without an MCP client. Custom tools render as constrained forms with validated inputs.
    ...
    ![workbench](https://raw.githubusercontent.com/ajgreyling/electric-elephant/main/docs/images/workbench/custom-tool.webp)

1. **Request Traces**: Inspect MCP requests to understand how AI agents interact with Electric Elephant.

    ![request-trace](https://raw.githubusercontent.com/ajgreyling/electric-elephant/main/docs/images/workbench/request-trace.webp)

## Summary

**Electric Elephant** is a zero-dependency, token efficient MCP server for relational databases with guardrails.
...
- **Vendor neutral**: Unlike Supabase MCP (Supabase-only) or MCP Toolbox (tilted toward Google Cloud), Electric Elephant works with any deployment—cloud, on-premise, or local.

- **Multi-database with consistent guardrails**: Not limited to a single Postgres database, but we don't expand beyond the database boundary either. This focus lets us provide consistent guardrails across all supported databases: read-only mode, row limits, connection/query timeouts, and (Electric Elephant) fail-closed PII/clinical heuristics on `execute_sql` unless `allow_access_to_pii_data` is explicitly enabled.

### The Bad

- **No platform integration**: MCP Toolbox integrates with Gemini and Google Cloud services. Supabase MCP connects to its hosted platform with OAuth. Electric Elephant is standalone.

- **No built-in authentication**: Unlike MCP Toolbox's Google auth support or Supabase's OAuth, Electric Elephant has no auth layer. You'll need a reverse proxy or network security for multi-tenant scenarios.
...
quadrantChart
    ...
    MCP Toolbox: [0.3, 0.9]
    Electric Elephant: [0.85, 0.6]
```

**If you're an AI-assisted developer working with local databases**, Electric Elephant is built for you. It's optimized for the local development workflow—zero setup friction, minimal token overhead, and no authentication complexity. Works with PostgreSQL, MySQL, SQL Server, MariaDB, and SQLite out of the box.

**If you need a vendor-neutral solution**, Electric Elephant is the only option that isn't tied to a specific cloud platform. Unlike Supabase MCP (Supabase-only) or MCP Toolbox (tilted toward Google Cloud), Electric Elephant works anywhere your database runs.
...
3. **Electric Elephant** (this article) - Zero-dependency, token efficient MCP server for PostgreSQL, MySQL, SQL Server, MariaDB, and SQLite
