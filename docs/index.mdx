...
- **Guardrails**: Read-only mode, row limiting, **5-minute default** query timeout (override with `query_timeout` in TOML), and optional PII/clinical column checks on `execute_sql` (wildcards and heuristic column matches blocked unless explicitly opted in)
