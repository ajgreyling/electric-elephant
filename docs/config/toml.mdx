...
<ParamField path="allow_access_to_pii_data" type="boolean" default="false">
  **execute_sql only.** When `false` or omitted, Electric Elephant may reject queries that project wildcards (`*` or `table.*`) or columns/expressions that match PII or sensitive clinical heuristics. Set to `true` only when policy explicitly allows returning that data.
...
| `allow_access_to_pii_data` | boolean | ❌ | Allow PII/clinical-sensitive projections for `execute_sql` (default: `false`) |
