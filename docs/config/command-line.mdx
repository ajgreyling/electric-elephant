...
- **CLI (single-DSN only):** `--allow-access-to-pii-data=true` overrides env when set. A bare `--allow-access-to-pii-data` does **not** enable access.
- **CLI (debugging, single-DSN only):** `--disable-pii-guard` (bare flag) or `--disable-pii-guard=true` / `=1` / `=yes` has the same effect as `--allow-access-to-pii-data=true`. If both are passed, `--allow-access-to-pii-data` is evaluated first (so `--allow-access-to-pii-data=false` keeps the guard on).
...
<ParamField path="--allow-access-to-pii-data" type="string" env="ALLOW_ACCESS_TO_PII_DATA">
  **Single-DSN mode only** (not used with `--config` / TOML). When set to the literal value `true`, allows `execute_sql` to bypass the PII/clinical projection guard. Prefer TOML per-source tool config when using multiple databases.
...
<ParamField path="--disable-pii-guard" type="boolean">
  **Single-DSN mode only.** Convenience flag for local debugging: a bare `--disable-pii-guard` or `--disable-pii-guard=true` (also `1` / `yes`) allows `execute_sql` to bypass the PII/clinical projection guard, same as `--allow-access-to-pii-data=true`. Use `=false` to leave the flag present but ineffective. Ignored when `--allow-access-to-pii-data` is set (that flag takes precedence).

  ```bash
  npx electric-elephant --dsn "postgres://..." --disable-pii-guard
  ```
</ParamField>
...
| `--allow-access-to-pii-data` | `ALLOW_ACCESS_TO_PII_DATA` | string | Single-DSN only: must be `=true` to allow sensitive projections on `execute_sql` |
| `--disable-pii-guard` | - | boolean | Single-DSN only: debug alias for allowing sensitive projections (bare or `=true` / `1` / `yes`) |
