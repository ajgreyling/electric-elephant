...
This guide will walk you through setting up Electric Elephant and connecting it to an AI tool for the first time. We'll use demo mode to get started quickly, then show you how to connect to your own database.
...
## Step 1: Start Electric Elephant in Demo Mode

<Note>
If using **stdio** transport, skip this step - your AI tool will spawn Electric Elephant as a subprocess automatically.
</Note>

For **http** transport, start Electric Elephant server manually:
...
<Tabs>
  ...
  <Tab title="Docker">
    ```bash
    docker run --rm -p 8080:8080 electric-elephant --transport http --port 8080 --demo
    ```
  </Tab>
</Tabs>
...
(ASCII startup banner printed to stderr)

v0.x.x [DEMO] - PostgreSQL-Only, Readonly-First MCP Server
...
<AccordionGroup>
  ...
  <Accordion icon="magnifying-glass" title="Query employee data">
    ...
    ```sql
    SELECT emp_no, first_name, last_name, salary FROM employees ORDER BY salary DESC LIMIT 5;
    ```

    <Note>
    With the default [PII/clinical guard](/tools/execute-sql#pii-and-clinical-data-guard), `SELECT *` may be rejected; explicit columns are preferred after inspecting the table with `search_objects`.
    </Note>
  </Accordion>
...
Electric Elephant provides an [agent skill](https://github.com/ajgreyling/electric-elephant/tree/main/skills/badger-db-mcp) that teaches your AI tool the best-practice workflow for database querying — explore the schema first, then write precise SQL. This reduces errors and back-and-forth.
...
Once you've verified Electric Elephant works in demo mode, connect it to your actual database.
