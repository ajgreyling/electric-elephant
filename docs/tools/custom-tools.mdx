...
<Warning>
**Constraints enforced at registration.** A custom tool whose `statement` violates any of these fails at startup (the server will not register it):

- **Single-schema scope** — the statement must stay within one schema; cross-schema and system-catalog (`information_schema`, `pg_catalog`) references are rejected. Callers pass a required `schema` argument (auto-added to every custom tool; you must not define a parameter named `schema`).
- **No health/clinical data** — HL7v2, FHIR, LOINC, SNOMED, and medical-style projections are hard-excluded.
- **No personal identifiers** — names, email, national IDs, DOB, address, etc. are hard-excluded. Only the user's mobile/phone number is permissible.
- **No wildcards** — `SELECT *` / `table.*` is rejected (it could expose the above). List explicit columns.

The examples below are written to satisfy these rules.
</Warning>
...
source = "production"
# Non-sensitive columns only — names/email would be rejected as hard PII.
statement = "SELECT id, status, created_at FROM users WHERE id = $1"
...
description = "Search employees by title with configurable result limit"
source = "prod_pg"
# Project non-PII columns only; you may filter on a name in WHERE (not projected).
statement = "SELECT emp_no, title, hire_date FROM employee WHERE title ILIKE '%' || $1 || '%' LIMIT $2"

[[tools.parameters]]
name = "search_term"
type = "string"
description = "Title to search for (case-insensitive partial match)"
...
source = "prod_pg"
statement = "SELECT id, customer_id, status, total, created_at FROM orders WHERE customer_id = $1 AND ($2::text IS NULL OR status = $2)"
...
Electric Elephant supports PostgreSQL-style numbered placeholders (`$1`, `$2`, …):
...
source = "postgres_db"
statement = "SELECT id, sku, title, price FROM products WHERE id = $1"
...
statement = "SELECT id, department, status FROM users WHERE ($1::text IS NULL OR department = $1) LIMIT $2"
...
source = "production"
statement = "SELECT id, customer_id, status, total, created_at FROM orders WHERE created_at BETWEEN $1 AND $2 ORDER BY created_at DESC"
...
The `readonly` setting on `execute_sql` only affects that tool. Custom tools are controlled by their SQL statement - Electric Elephant analyzes the statement to determine if it's read-only.
...
statement = "SELECT id, department, status FROM users WHERE active = $1 LIMIT $2"
...
- No parameter may be named `schema` (reserved for the auto-added target-schema argument)
- The `statement` must be single-schema scoped (no cross-schema or system-catalog references)
- The `statement` must not project health/clinical data, personal identifiers, or wildcards (`SELECT *`)
