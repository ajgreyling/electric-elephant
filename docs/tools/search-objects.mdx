...
- `table` (optional): Filter by table name (requires `schema`; only valid for `object_type: "column"` or `object_type: "index"`)
...
```json
{
  ...
  "table": "string (optional; requires schema; column/index only)",
