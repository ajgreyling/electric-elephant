...
- `schema` (required): Target schema for this search (all results are limited to this schema)
- `pattern` (optional): Search pattern using SQL LIKE syntax (`%` for wildcard, `_` for single character). Defaults to `"%"` (match all)
- `table` (optional): Filter by table name (only valid for `object_type: "column"` or `object_type: "index"`)
...
**Breaking change:** `schema` is required on every call. Cross-schema discovery is not supported.

## Input Schema (JSON)

```json
{
  "object_type": "schema | table | column | procedure | function | index",
  "schema": "string (required)",
  "pattern": "string (optional, default: \"%\")",
  "table": "string (optional; column/index only)",
  "detail_level": "names | summary | full (optional, default: names)",
...
```json Search columns in one table
{
  "object_type": "column",
  "schema": "public",
  "table": "users",
...
```json Search indexes in one table
{
  "object_type": "index",
  "schema": "public",
  "table": "users",
