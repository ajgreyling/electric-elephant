...
describe("loadConnectors", () => {
  ...
  it("should log and continue when a transitive dependency is missing", async () => {
    const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
    ...
    await loadConnectors([
      { load: () => Promise.reject(transitiveErr), name: "PostgreSQL", driver: "pg" },
    ]);

    expect(errorSpy).toHaveBeenCalledWith(
      'Skipping PostgreSQL connector: required dependency "@azure/core-client" not installed.'
    );
    errorSpy.mockRestore();
  });
