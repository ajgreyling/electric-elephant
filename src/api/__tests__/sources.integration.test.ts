...
import { ConnectorManager as CM } from "../../connectors/manager.js";
...
describe("Data Sources API Integration Tests", () => {
  ...
  beforeAll(async () => {
    ...
    container = await new PostgreSqlContainer("postgres:15-alpine")
      .withDatabase("fixturedb")
      .withUsername("fixuser")
      .withPassword("fixpass")
      .start();
    ...
    const sources: SourceConfig[] = [
      ...
      { id: "writable_limited", type: "postgres", dsn },
      { id: "writable_unlimited", type: "postgres", dsn },
    ];

    const tools: ToolConfig[] = [
      { name: "execute_sql", source: "readonly_limited", readonly: true, max_rows: 100 },
      { name: "execute_sql", source: "writable_limited", readonly: false, max_rows: 500 },
    ];
  ...
  describe("GET /api/sources", () => {
    ...
    it("should include database type postgres for all sources", async () => {
      const response = await fetch(`${BASE_URL}/api/sources`);
      const sources = (await response.json()) as DataSource[];
    ...
    it("should include database connection details", async () => {
      ...
      sources.forEach((source) => {
        expect(source.database).toBe("fixturedb");
  ...
  describe("GET /api/sources/{source-id}", () => {
    ...
    it("should handle URL-encoded source IDs", async () => {
      const response = await fetch(
        `${BASE_URL}/api/sources/${encodeURIComponent("readonly_limited")}`
      );
      expect(response.status).toBe(200);
    ...
    it("should include correct tool name for specific source", async () => {
      ...
      expect(source.tools[0].description).toContain("postgres");
    });
